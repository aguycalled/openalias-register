    ...
    request({ url: util.format(config.url, req.body.email+".nav.community", "txt"), headers: { "Authorization": auth } }, function (err, response, body) {
      if(err){
        ...
        if(!req.body.signature) {
          if(r.length > 0) {
            res.render('index', {step: 1, error: 'This username is already registered! Provide a cryptographic signature to update it.', address: req.body.address, email: req.body.email});
          } else {
            res.render('index', {step: 1, error: '', address: req.body.address, email: req.body.email});
          }
        } else {
          try {
            var boolMessage = Message(req.body.email+'@nav.community').verify(req.body.address, req.body.signature);
          }catch(e){
            res.render('index', {step: 1, error: 'Wrong signature', address: req.body.address, email: req.body.email});
            return;
          }
          if(!boolMessage) {
            res.render('index', {step: 1, error: 'Wrong signature', address: req.body.address, email: req.body.email});
            ...
            request({ url: util.format(config.url, req.body.email+".nav.community", "txt"), method: "POST", headers: { "Authorization": auth }, body: "oa1:nav recipient_address="+req.body.address+";" }, function (err, response, body) {
              if(err){
                res.render('index', {step: 1, error: 'Internal error', address: req.body.address, email: req.body.email});
              } else {
